...
pub async fn create_facility(
    ...
    new_facility: web::Json<Facility>,
) -> Result<HttpResponse, Error> {
    let facility = use_case
        .facility_use_case
        .create(new_facility.into_inner())
        .await
        .map_err(|e| {
...
pub async fn get_facility_by_id(
    ...
    match use_case.facility_use_case.get_by_id(&id).await {
        ...
        Err(e) if e.to_lowercase().contains("not found") => {
            Ok(HttpResponse::NotFound().body("Facility not found"))
        }
        Err(e) => {
...
pub async fn update_facility(
    use_case: web::Data<AppState>,
    id: web::Path<String>,
    updated_facility: web::Json<Facility>,
) -> Result<HttpResponse, Error> {
