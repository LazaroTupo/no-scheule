...
pub async fn create_schedule(
    ...
    new_schedule: web::Json<Schedule>,
) -> Result<HttpResponse, Error> {
    let schedule = use_case
        .schedule_use_case
        .create(new_schedule.into_inner())
        .await
        .map_err(|e| {
...
pub async fn get_schedule_by_id(
    ...
    match use_case.schedule_use_case.get_by_id(&id).await {
        ...
        Err(e) if e.to_lowercase().contains("not found") => {
            Ok(HttpResponse::NotFound().body("Schedule not found"))
        }
        Err(e) => {
...
pub async fn update_schedule(
    use_case: web::Data<AppState>,
    id: web::Path<String>,
    updated_schedule: web::Json<Schedule>,
) -> Result<HttpResponse, Error> {
    ...
    match use_case.schedule_use_case.update(&schedule).await {
        ...
        Err(e) if e.to_lowercase().contains("not available") => {
            Ok(HttpResponse::Conflict().body(e))
        }
        Err(e) => {
