...
pub async fn register_extracourse(
    ...
    use_case
        .course_use_case
        .register_extracourse(course_data, schedule)
        .await
        .map_err(|e| {
...
pub async fn get_course_by_id(
    ...
    match use_case.course_use_case.get_by_id(&id).await {
        ...
        Err(e) if e.to_lowercase().contains("not found") => {
            Ok(HttpResponse::NotFound().body("Course not found"))
        }
        Err(e) => {
...
pub async fn get_courses_by_user(
    ...
    match use_case.course_use_case.get_by_user(&user_id).await {
        ...
        Err(e) if e.to_lowercase().contains("not found") => {
            Ok(HttpResponse::NotFound().body("Courses not found for user"))
        }
        Err(e) => {
